...
string Cube::cube_hash(Point& p) { // applying hash function 
	std::forward_list<float *>::iterator it_v = v.begin();
	std::forward_list<float>::iterator it_t = t.begin();
	std::forward_list<int>::iterator it_r = r.begin();
...
void Cube::fill(std::vector<Point *>& points) {
	for (int i = 0; i < points.size(); ++i)
	{
		cube.insert(make_pair(cube_hash(*(points[i])),points[i]));
	}

}

DPnt Cube::NN(Point * query,double(*metric)(Point *,Point *),int probes, int M, double R,vector<DPnt>& nns) {
	unsigned long buc = cube.bucket(cube_hash(*query));
	/*cout << "Bucket of " << query->get_name();
	query->print_coords();*/
